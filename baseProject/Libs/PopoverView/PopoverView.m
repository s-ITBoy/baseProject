...
#define RGBAl(r, g, b)    [UIColor colorWithRed:(r)/255.f green:(g)/255.f blue:(b)/255.f alpha:1.f]
...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        // Initialization code
        self.borderColor = [UIColor whiteColor];// RGB(200, 199, 204);
...
//        CGFloat width =  [title sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
        CGFloat width = [title ss_boundingRectwithSize:CGSizeMake(300, 100) withFont:[UIFont systemFontOfSize:15]].width;
    ...
    if ([self.titleArray count] == [self.imageArray count]) {
        ...
        frame.size.width = 10 + frame.size.width + 60;
    }
    CGFloat wWidth = [UIApplication sharedApplication].keyWindow.frame.size.width;
    frame.origin.x = self.showPoint.x - frame.size.width/2;
    if( wWidth - CGRectGetMaxX(frame) <= 1){
        frame.origin.x = self.showPoint.x - frame.size.width/2 - 30.0f;
    }else{
        frame.origin.x = self.showPoint.x - frame.size.width/2;
    }
